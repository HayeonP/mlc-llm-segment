...
void SegmentRunner::Init(std::string model, tvm::Device& device, std::string model_lib, std::string mode){
  _model = model;
  ...
  auto create_segment_runner_engine_func_ = tvm::ffi::Function::GetGlobal("mlc.serve.segment_runner.create_segment_runner_engine");
  if(!create_segment_runner_engine_func_.has_value()){
    std::cout<<"[ERROR] Cannot create segment_runner engine"<<std::endl;
    exit(0);
  }
  tvm::ffi::Function create_segment_runner_engine_func = create_segment_runner_engine_func_.value();
  _engine_module = create_segment_runner_engine_func().cast<tvm::runtime::Module>();
...
void SegmentRunner::Request(std::string& prompt, int max_tokens){
  ...
  ***********************************************************/
  ...
  _trace_recorder.value()->AddEvent(_request_id.value(), std::string("receive request"));
  ...
  // - Check prompt length
  _trace_recorder.value()->AddEvent(_request_id.value(), std::string("start tokenization"));
  ...
  _trace_recorder.value()->AddEvent(_request_id.value(), std::string("finish tokenization"));
  ...
  ***********************************************************/
  ...
  generation_config_node->logprobs = _request.logprobs;
  generation_config_node->top_logprobs = _request.top_logprobs;
  ...
  _trace_recorder.value()->AddEvent(_request_id.value(), std::string("invoke generate"));
 ...
 // _generate()
  ...
  mlc::llm::serve::Request request_for_add_request_event = Downcast<mlc::llm::serve::Request>(create_request_rv);

  /***********************************************************
    - Send request to the engine
  ***********************************************************/
 _trace_recorder.value()->AddEvent(_request_id.value(), std::string("Add request"));
  ...
  add_request_func(request_for_add_request_event);
...
std::string SegmentRunner::Execute(){
  ...
  Array<Optional<String>> finish_reasons(_request.n, Optional<String>());

  while(generate_segment_output.move_next()){
    std::vector<CallbackStreamOutput> delta_outputs = generate_segment_output.current_value();

    bool use_function_calling = false; // TODO: use_function_calling is always "false" for now.
    std::optional<ChatCompletionStreamResponse> response = process_chat_completion_stream_output(delta_outputs, _request, _request_id, false, finish_reasons);
    ...
    if(response.has_value()){
      ...
      output_response_list.push_back(output_response);
    }
  }
  ...
  std::vector<std::string> final_output_texts(_request.n);
  std::vector<std::string> final_finish_reasons(_request.n);
  ...
  for(auto& response : output_response_list){
    for(auto choice : response.choices){
      if(choice.delta.content.IsNull()) continue;

      final_output_texts[choice.index] += choice.delta.content.Text();
  ...
  std::string request_id_str(_request_id.value());
  ChatCompletionResponse chat_completion_response = wrap_chat_completion_response(request_id_str, _model, final_output_texts, final_finish_reasons); // TODO: Doesn't support logprob, funciton call, tool calls for now  

  return _response_to_str(chat_completion_response);
}

Generator<std::vector<CallbackStreamOutput>> SegmentRunner::_generate_segment_output(){
...
ChatCompletionResponse SegmentRunner::wrap_chat_completion_response(std::string& request_id, std::string& model, std::vector<std::string>& output_texts, std::vector<std::string>& finish_reasons){
...
std::optional<ChatCompletionStreamResponse> SegmentRunner::process_chat_completion_stream_output(std::vector<CallbackStreamOutput>& delta_outputs, mlc::llm::json_ffi::ChatCompletionRequest& request, Optional<String> request_id, bool use_function_calling, Array<Optional<String>> finish_reasons){
...
bool SegmentRunner::IsEnd(){
  return _is_end;
}

void SegmentRunner::_request_stream_callback_impl(std::vector<mlc::llm::serve::RequestStreamOutput> delta_outputs, std::vector<std::vector<CallbackStreamOutput>>& output_request_outputs, Optional<String>& output_request_final_usage_json_str){  
...
void SegmentRunner::_check_engine_config(std::string model, std::string model_lib, EngineMode mode, mlc::llm::serve::EngineConfig engine_config){
...
std::vector<ModelInfo> SegmentRunner::_parse_members(std::string model, std::string model_lib){
...
void SegmentRunner::_convert_model_info(ModelInfo model, Conversation& conversation, std::vector<std::string>& config_file_paths, std::string& output_model_path, std::string& output_model_lib){
...
std::vector<ModelInfo> SegmentRunner::_parse_models(std::string model, std::string model_lib){ // Doesn't support addtional models
  std::vector<ModelInfo> models;
  ModelInfo model_info;
  model_info.model = model;
  model_info.model_lib = model_lib;
...
void SegmentRunner::_process_model_args(std::vector<ModelInfo>& models, tvm::Device& device, mlc::llm::serve::EngineConfig& engine_config, std::vector<ModelArg>& output_model_args, std::vector<std::string>& output_config_file_paths, Conversation& output_conv_template){
...
void SegmentRunner::_sync_request_stream_callback(tvm::ffi::Array<mlc::llm::serve::RequestStreamOutput> delta_outputs){
  _sync_output_queue.put_nowait(delta_outputs);
}

ChatCompletionRequest SegmentRunner::_create_chat_completion_request(std::string& model, std::string prompt, int max_tokens, bool stream){
...
std::string SegmentRunner::_response_to_str(ChatCompletionResponse& response){
