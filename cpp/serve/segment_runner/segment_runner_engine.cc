 ...
 */
#include "segment_runner_engine.h"
...
/*! \brief The segment runner engine instruction kind. */
...
class SegmentRunnerEngineImpl : public SegmentRunnerEngine {
 public:
  void InitSegmentRunnerEngine(Device device, Optional<Function> request_stream_callback,
                          Optional<EventTraceRecorder> trace_recorder) final {
    device_ = device;
    CHECK(request_stream_callback.defined())
        << "SegmentRunnerEngine requires request stream callback function, but it is not given.";
  ...
  void AbortRequest(const String& request_id) final {
    ...
    if (background_engine_ != nullptr) {
      background_engine_->AbortRequest(Downcast<String>(request_id));
    }
    if (background_engine_ != nullptr) {      
      background_engine_->Step();                
    }
 ...
 private:
  ...
  std::condition_variable activated_cv_;
  std::condition_variable loop_done_cv_;
...
/*! \brief The implementation of SegmentRunnerEngine. */
class SegmentRunnerEngineModule : public SegmentRunnerEngineImpl, public ModuleNode {
 public:
  TVM_MODULE_VTABLE_BEGIN("mlc.serve.segment_runner_engine");
  TVM_MODULE_VTABLE_ENTRY("init_segment_runner_engine", &SegmentRunnerEngineImpl::InitSegmentRunnerEngine);
  TVM_MODULE_VTABLE_ENTRY("reload", &SegmentRunnerEngineImpl::Reload);
  TVM_MODULE_VTABLE_ENTRY("add_request", &SegmentRunnerEngineImpl::AddRequest);
  ...
  TVM_MODULE_VTABLE_ENTRY("reset", &SegmentRunnerEngineImpl::Reset);
  TVM_MODULE_VTABLE_ENTRY("debug_call_func_on_all_worker",
                          &SegmentRunnerEngineImpl::DebugCallFuncOnAllAllWorker);
  
  TVM_MODULE_VTABLE_END();
};

TVM_FFI_REGISTER_GLOBAL("mlc.serve.segment_runner.create_segment_runner_engine").set_body_typed([]() {
  return Module(make_object<SegmentRunnerEngineModule>());
});

std::unique_ptr<SegmentRunnerEngine> SegmentRunnerEngine::Create() {
  std::unique_ptr<SegmentRunnerEngineImpl> threaded_engine = std::make_unique<SegmentRunnerEngineImpl>();
  return std::move(threaded_engine);
}

}  // namespace segment_runner
}  // namespace llm
}  // namespace mlc
